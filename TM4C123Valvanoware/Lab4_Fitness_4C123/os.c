...
void SetInitialStack(int i){
  ...
  // **Same as Lab 2 and Lab 3****
	...
	Stacks[i][STACKSIZE-15] = 0x04040404;
}
...
// If there are multiple highest priority (not blocked, not sleeping) run these round robin
	...
	max = 255;
	do{
		if((pt->priority <max)&&(pt->sleep==0)&&(pt->blocked==0)){
			max=pt->priority;
			bestpt = pt;
		}
		pt = pt->next;
...
//***IMPLEMENT THIS***
	SYSCTL_RCGCGPIO_R |= 0x20;// 1) activate clock for Port D
// allow time for clock to stabilize
	//GPIO_PORTD_LOCK_R = 0x4C4F434B;// 2) no need to unlock PD6
	GPIO_PORTD_CR_R =0x40; // 3) disable analog on PD6
	GPIO_PORTD_AMSEL_R &= ~0x40;// 4) configure PD6 as GPIO
	...
	NVIC_PRI0_R = (NVIC_PRI0_R&0x00FFFFFF)|(priority<<28);// priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
